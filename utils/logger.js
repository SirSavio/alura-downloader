...
function log(type, infos) {
    ...
    const lines = {
        1: ansi.yellow(`[DEBUG] LOGIN IN ${ansi.yellowBright(`${infos.email}:${infos.password}`.toUpperCase())}`),
        2: ansi.red(`[INFO] ERROR CONNECTING ${ansi.yellow(`${infos.email}:${infos.password}`.toUpperCase())}`),
        ...
        6: ansi.green(`[INFO] LOGIN SUCCESS ${ansi.greenBright(`${infos.email}:${infos.password}`.toUpperCase())}`),
